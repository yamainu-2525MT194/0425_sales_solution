...
credentials = ServiceAccountCredentials.from_json_keyfile_name("salesapisheet-e10ba3c891e3.json", scope)
...
if sheet_url and "export?format=csv" in sheet_url:
    try:
        ...
        # ② ロジスティック回帰（提案力判定）
        st.subheader("📉 ② 提案力分類（高/低）：Logistic Regression")
        ...
        # ④ 重要特徴量の可視化
        st.subheader("📊 ④ 成果に寄与する指標（RFによる特徴量）")
        importance = pd.Series(rf_model.feature_importances_, index=X.columns)
        st.bar_chart(importance)

        # ⑤ LightGBM による回帰モデル（代替DL）
        st.subheader("🚀 ⑤ LightGBMによる高精度予測")
        ...
        st.subheader("📌 ⑤ 営業別の行動アドバイス（AIコーチング）")
